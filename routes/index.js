...
router.post('/taxi/login', function (req, res, next) {
  ...
  db.query(queryStr, (err, rows, fields) => {
    if (!err) {
      ...
      if (code == 0) { updateFcm(fcmToken, 'tb_user', 'user_id', userId) }
  ...
  db.query(queryStr, function (err, rows, fields) {
    if (!err) {
      ...
      if (err.code == "ER_DUP_ENTRY") {
        res.json([{ code: 2, message: "이미 등록된 ID입니다.", data: err }])
      } else {
...
router.post('/driver/login', function (req, res, next) {
  ...
  db.query(queryStr, (err, rows, fields) => {
    if (!err) {
      ...
      if (code == 0) { updateFcm(fcmToken, 'tb_driver', 'driver_id', userId) }
...
const sendFcm = (fcmToken, msg) => {
  ...
  admin.messaging().send(message)
    ...
    .catch((error) => {
      console.log("-- push erreo / " + JSON.stringify(error))
    })
}
